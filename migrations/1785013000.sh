...
# Arch uses pacman -Qo; Fedora uses rpm -qf. Either means leave it alone.
if command -v pacman >/dev/null 2>&1 && pacman -Qo "$zram_conf" &>/dev/null; then
  exit 0
fi
if command -v rpm >/dev/null 2>&1 && rpm -qf "$zram_conf" &>/dev/null; then
  exit 0
fi
